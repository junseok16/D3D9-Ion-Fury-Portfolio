...
int32 CPowerMonitor::Update(const float& _fDeltaSeconds)
{
...
int32 CPowerMonitor::LateUpdate(const float& _fDeltaSeconds)
{
    return CInteractiveBox::LateUpdate(_fDeltaSeconds);
}

void CPowerMonitor::Render()
{
    CInteractiveBox::Render();
}

void CPowerMonitor::EndPlay()
{
    CInteractiveBox::EndPlay();
}

void CPowerMonitor::Release()
{
    CInteractiveBox::Release();
}

int32 CPowerMonitor::OnCollisionEnter(std::weak_ptr<CColliderComponent> _wpSrcCollider, std::weak_ptr<CColliderComponent> _wpDstCollider)
{
    return 0;
}

void CPowerMonitor::OnCollisionStay(std::weak_ptr<CColliderComponent> _wpSrcCollider, std::weak_ptr<CColliderComponent> _wpDstCollider)
{
}

void CPowerMonitor::OnCollisionExit(std::weak_ptr<CColliderComponent> _wpSrcCollider, std::weak_ptr<CColliderComponent> _wpDstCollider)
{
}

void CPowerMonitor::KeyInput(const float& _fDeltaSeconds)
{
	FVector3 vDist = m_wpPlayerActor.lock()->GetActorWorldPosition() - m_wpPowerMonitor.lock()->GetOwnerActor().lock()->GetActorWorldPosition();
	float fDist = D3DXVec3Length(&vDist);

	m_wpOutGameUI.lock()->SetPowerMonitorActor(fDist);
...
void CPowerMonitor::InGameSetting()
{
...
void CPowerMonitor::SetPowerMonitorFrame()
{
    ...
    if (m_bPowerMonitor == true && m_wpPowerMonitor.lock()->GetCurFrame() == iEndFrame)
    {
        ...
        //m_wpPlayerActor.lock()->SetSandevistanEnable();
        m_wpPlayerActor.lock()->SetBlinkEnable();
    }  
